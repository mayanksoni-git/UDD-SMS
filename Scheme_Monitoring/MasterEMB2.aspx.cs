...
public partial class MasterEMB2 : System.Web.UI.Page
{
    ...
    protected void btnEdit_Click(string _Status)
    {
        DataSet ds = new DataSet();
        ...
        {
            get_tbl_PackageEMB(Convert.ToInt32(hf_ProjectWork_Id.Value), _Status);
        }

    }
    private void get_tbl_PackageEMB(int Package_Id, string _Status)
    {
        DataSet dsFull = new DataSet();
        dsFull = (new DataLayer()).get_tbl_PackageEMB(Package_Id, 0, "", true);

        DataSet ds = new DataSet();
        ds = FilterData(dsFull, _Status);
    ...
    private DataSet FilterData(DataSet dsFull, string _Status)
    {
        int RowCont = dsFull.Tables[0].Rows.Count;
        ...
        DataView dv = new DataView(dsFull.Tables[0]);
        dv.RowFilter = "PackageBOQ_Sr_No >= " + Start + " and PackageBOQ_Sr_No <= " + End + "";
        DataSet ds = new DataSet();
    ...
    protected void btnSaveEMB_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                if ((new DataLayer()).Insert_tbl_PackageEMB(null, null, obj_tbl_PackageEMB_Li, null, Convert.ToInt32(Session["Person_BranchOffice_Id"].ToString()), Convert.ToInt32(Session["PersonJuridiction_DesignationId"].ToString()), Convert.ToInt32(hf_Project_Id.Value), "O", 0))
                {
    ...
    protected void btnSaveAndForward_Click(object sender, EventArgs e)
    {
        ...
        for (int j = 0; j < grdEMB.PageCount; j++)
        {
            ...
            for (int i = 0; i < grdEMB.Rows.Count; i++)
            {
                ...
                {
                    PercentageToBeReleased = 0;
                    MessageBox.Show("Error In Percentage at Sr No: " + (i + 1).ToString());
                    return;
                }
                obj_tbl_PackageEMB.PackageEMB_GSTType = grdEMB.Rows[i].Cells[24].Text.Replace("&nbsp;", "").Trim(); ;
                try
        ...
        else
        {
            ...
            {
                if ((new DataLayer()).Insert_tbl_PackageEMB(obj_tbl_PackageEMB_Master, obj_tbl_PackageEMBApproval, obj_tbl_PackageEMB_Li, obj_tbl_PackageEMBAdditional_Li, Convert.ToInt32(Session["Person_BranchOffice_Id"].ToString()), Convert.ToInt32(Session["PersonJuridiction_DesignationId"].ToString()), Convert.ToInt32(hf_Project_Id.Value), "O", 0))
                {
                    MessageBox.Show("Package EMB Created Successfully!");
                    reset();
                    return;
    ...
    protected void btnLoadFirstHalf_Click(object sender, EventArgs e)
    {
        if (Request.QueryString.Count > 0)
        {
            hf_ProjectWork_Id.Value = Request.QueryString["ProjectWork_Id"].ToString();
            hf_Project_Id.Value = Request.QueryString["Project_Id"].ToString();
            hf_ProjectWorkPkg_LastRABillNo.Value = Request.QueryString["LastRABillNo"].ToString();

            btnEdit_Click("First");
        }
    }

    protected void btnLoadSecondHalf_Click(object sender, EventArgs e)
    {
        if (Request.QueryString.Count > 0)
        {
            hf_ProjectWork_Id.Value = Request.QueryString["ProjectWork_Id"].ToString();
    ...
    protected void btnLoadFull_Click(object sender, EventArgs e)
    {
        if (Request.QueryString.Count > 0)
        {
            hf_ProjectWork_Id.Value = Request.QueryString["ProjectWork_Id"].ToString();
