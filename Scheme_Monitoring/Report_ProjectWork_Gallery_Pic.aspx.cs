...
public partial class Report_ProjectWork_Gallery_Pic : System.Web.UI.Page
{
    ...
    protected void btnDownloadDistrictWise_Click(object sender, EventArgs e)
    {
        int District_Id = 0;
        int Zone_Id = 0;
        int Circle_Id = 0;
        ...
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_Project_Work_Gallery_Upload_DetailsPath(Scheme_Id, Zone_Id, Circle_Id, Division_Id, District_Id, ULB_Id, ChkShowPIC_NA.Checked);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            try
            {
                string path = Server.MapPath(".") + @"\Downloads\Gallery\";

                if (Directory.Exists(path + "District"))
                {
                    Directory.Delete(path + "District", true);
                }
                Directory.CreateDirectory(path + "District");
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    string Dist_Name = ds.Tables[0].Rows[i]["Jurisdiction_Name_Eng"].ToString().Replace("-", "_").Replace("(", "_").Replace(")", "_").Trim();
                    string MOUD = ds.Tables[0].Rows[i]["ProjectWork_ProjectCode"].ToString().Replace("-", "_").Replace("(", "_").Replace(")", "_").Trim();
                    FileInfo fl = new FileInfo(Server.MapPath(".") + ds.Tables[0].Rows[i]["ProjectWorkGallery_Path"].ToString());
                    if (fl.Exists)
                    {
                        if (!Directory.Exists(path + "District" + "\\" + Dist_Name))
                        {
                            Directory.CreateDirectory(path + "District" + "\\" + Dist_Name);
                        }
                        if (!Directory.Exists(path + "District" + "\\" + Dist_Name + "\\" + MOUD))
                        {
                            Directory.CreateDirectory(path + "District" + "\\" + Dist_Name + "\\" + MOUD);
                        }
                        fl.CopyTo(path + "District\\" + Dist_Name + "\\" + MOUD + "\\" + fl.Name, true);
                    }
                }

                if (File.Exists(path + "District" + ".zip"))
                {
                    File.Delete(path + "District" + ".zip");
                }

                ZipFile.CreateFromDirectory(path + "District", path + "District" + ".zip");
                //Download File
                //System.Web.HttpResponse response = System.Web.HttpContext.Current.Response;
                //response.ClearContent();
                //response.Clear();
                //response.ContentType = "application/x-zip-compressed";
                //response.AddHeader("Content-Disposition", "attachment; filename=District.zip;");
                //response.TransmitFile(path + "District" + ".zip");
                //response.Flush();
                //response.End();

                divLink.InnerHtml = "<a target='_blank' href='"+ Session["Base_Web_URL"].ToString() + "Downloads/Gallery/District.zip'>Download</a>";
    ...
    protected void btnDownloadZoneWise_Click(object sender, EventArgs e)
    {
        int District_Id = 0;
        int Zone_Id = 0;
        int Circle_Id = 0;
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            try
            {
                ...
                if (Directory.Exists(path + "Zone"))
                {
                    Directory.Delete(path + "Zone", true);
                }
                Directory.CreateDirectory(path + "Zone");
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    string Zone_Name = ds.Tables[0].Rows[i]["Zone_Name"].ToString().Replace("-", "_").Replace("(", "_").Replace(")", "_").Trim();
                    string MOUD = ds.Tables[0].Rows[i]["ProjectWork_ProjectCode"].ToString().Replace("-", "_").Replace("(", "_").Replace(")", "_").Trim();
                    FileInfo fl = new FileInfo(Server.MapPath(".") + ds.Tables[0].Rows[i]["ProjectWorkGallery_Path"].ToString());
                    if (fl.Exists)
                    {
                        if (!Directory.Exists(path + "Zone" + "\\" + Zone_Name))
                        {
                            Directory.CreateDirectory(path + "Zone" + "\\" + Zone_Name);
                        }
                        if (!Directory.Exists(path + "Zone" + "\\" + Zone_Name + "\\" + MOUD))
                        {
                            Directory.CreateDirectory(path + "Zone" + "\\" + Zone_Name + "\\" + MOUD);
                        }
                        fl.CopyTo(path + "Zone\\" + Zone_Name + "\\" + MOUD + "\\" + fl.Name, true);
                    }
                }

                if (File.Exists(path + "Zone" + ".zip"))
                {
                    File.Delete(path + "Zone" + ".zip");
                }

                ZipFile.CreateFromDirectory(path + "Zone", path + "Zone" + ".zip");
                //Download File
                //System.Web.HttpResponse response = System.Web.HttpContext.Current.Response;
                //response.ClearContent();
                //response.Clear();
                //response.ContentType = "application/x-zip-compressed";
                //response.AddHeader("Content-Disposition", "attachment; filename=Zone.zip;");
                //response.TransmitFile(path + "Zone" + ".zip");
                //response.Flush();
                //response.End();

                divLink.InnerHtml = "<a target='_blank' href='"+ Session["Base_Web_URL"].ToString() + "Downloads/Gallery/Zone.zip'>Download</a>";
                divLink.Visible = true;
            }
            catch (Exception eee)
            {
