...
public partial class Report_ProjectWorkPkg_LD_Imposed : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString.Count > 0)
            {
                ...
                try
                {
                    LDWithdraw = Convert.ToInt32(Request.QueryString["LDWithdraw"].ToString());
                }
                catch
                {
                    LDWithdraw = -1;
                ...
                try
                {
                    ...
                    ddlScheme.SelectedValue = Scheme_Id;
                }
                catch
                {
                    Scheme_Id = "0";
                ...
                try
                {
                    ...
                    ddlSearchZone.SelectedValue = District_Id.ToString();
                    ddlSearchZone_SelectedIndexChanged(ddlSearchZone, new EventArgs());
                }
                catch
                {
                    Zone_Id = 0;
                ...
                try
                {
                    ...
                    ddlSearchCircle.SelectedValue = District_Id.ToString();
                    ddlSearchCircle_SelectedIndexChanged(ddlSearchCircle, new EventArgs());
                }
                catch
                {
                    Circle_Id = 0;
                ...
                if (LDWithdraw == 1)
                {
                    chkRemoveSelected.Checked = true;
                }
            }

            get_Liquidated_Damages_Details();
        }
    }
    ...
    protected void get_Liquidated_Damages_Details()
    {
        int District_Id = 0;
        int Zone_Id = 0;
        int Circle_Id = 0;
        ...
        ds = (new DataLayer()).get_Liquidated_Damages_Details(Scheme_Id, Zone_Id, Circle_Id, Division_Id, District_Id, ULB_Id, chkRemoveSelected.Checked);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
