...
public partial class Report_ProjectWork_Pre_Closure : System.Web.UI.Page
{
    ...
    protected void get_Project_Work_Pre_Close()
    {
        ...
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_Project_Work_Pre_Close(Scheme_Id, Zone_Id, Circle_Id, Division_Id, District_Id, ULB_Id, 1);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
        ...
        ds = (new DataLayer()).get_Project_Work_Pre_Close(Scheme_Id, Zone_Id, Circle_Id, Division_Id, District_Id, ULB_Id, 2);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
        ...
        ds = (new DataLayer()).get_Project_Work_Pre_Close(Scheme_Id, Zone_Id, Circle_Id, Division_Id, District_Id, ULB_Id, 3);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            grdFinancialPartial.DataSource = ds.Tables[0];
            grdFinancialPartial.DataBind();

            List<string> _columnsList = new List<string>();

            for (int i = 0; i < grdFinancialPartial.Columns.Count; i++)
            {
                if (grdFinancialPartial.Columns[i].Visible == true)
                {
                    _columnsList.Add(null);
                }
            }
        ...
        {
            grdFinancialPartial.DataSource = null;
            grdFinancialPartial.DataBind();
        }
    }
    ...
    protected void grdFinancialPartial_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.Header)
        {
            e.Row.Cells[3].Text = Session["Default_Zone"].ToString();
