...
public partial class Dashboard_CM_District : System.Web.UI.Page
{    
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            get_PMIS_Dashboard_View(District_Id, Scheme_Id);
            get_tbl_ProjectWork_CM_District(District_Id, Scheme_Id);
        }
    }
    protected void get_PMIS_Dashboard_View(int District_Id, string Scheme_Id)
    {
    ...
    private void get_tbl_ProjectWork_CM_District(int District_Id, string Scheme_Id)
    {
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_tbl_ProjectWork_CM_District(District_Id, Scheme_Id);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
