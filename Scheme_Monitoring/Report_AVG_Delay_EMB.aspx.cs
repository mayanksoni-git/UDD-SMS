...
public partial class Report_AVG_Delay_EMB : System.Web.UI.Page
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        get_Average_Delay_EMB_Report();
    }

    private void get_Average_Delay_EMB_Report()
    {
        int Project_Id = 0;
        int Zone_Id = 0;
        int Circle_Id = 0;
        ...
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_Average_Delay_EMB_Report(Project_Id, Zone_Id, Circle_Id, Division_Id, txtDateFrom.Text, txtDateTill.Text, 0, 0);
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            int Total_MB = Convert.ToInt32(ds.Tables[0].Compute("sum(Total_MB)", "").ToString());
            int Total_Diff = Convert.ToInt32(ds.Tables[0].Compute("sum(AVG_Diff)", "").ToString());
            int Final_AVG = 0;
            try
            {
                Final_AVG = Convert.ToInt32(Total_Diff / Total_MB);
            }
            catch
            {
                
            }
            grdPost.FooterRow.Cells[11].Text = Total_MB.ToString();
            grdPost.FooterRow.Cells[12].Text = Final_AVG.ToString();
        }
        else
        {
    ...
    protected void btnView_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            Person_Id = Convert.ToInt32(gr.Cells[3].Text.Trim());
        }
        catch
        {
            Person_Id = 0;
        ...
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_Average_Delay_EMB_Detailed(Person_Id, Project_Id, Zone_Id, Circle_Id, Division_Id, txtDateFrom.Text, txtDateTill.Text, 0, 0);
