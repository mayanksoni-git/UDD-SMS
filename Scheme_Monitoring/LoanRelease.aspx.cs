...
public partial class LoanRelease : System.Web.UI.Page
{
    ...
    private tbl_LoanRelease CreateLoanReleaseObject()
    {
        ...
        if (!double.TryParse(txtReleaseAmount.Text, out releaseAmount))
        {
            releaseAmount = 0; // Default to 0 if parsing fails
        }

        if (!int.TryParse(txtInstNo.Text, out instNo))
        {
            instNo = 0; // Default to 0 if parsing fails
        }
        ...
        return new tbl_LoanRelease
        {
            ...
            InstNo = instNo,
            ReleaseDate = txtReleaseDate.Text.ToString().ToDate()
        };
    }
    ...
    public bool ValidateFields()
    {
        ...
        if (string.IsNullOrWhiteSpace(txtReleaseAmount.Text))
        {
            MessageBox.Show("Please enter Release Amount!");
            txtReleaseAmount.Focus();
            return false;
        }
        if (string.IsNullOrWhiteSpace(txtInstNo.Text))
        {
            MessageBox.Show("Please enter installment no !");
            txtInstNo.Focus();
            return false;
        }
        if (string.IsNullOrWhiteSpace(txtReleaseDate.Text))
        {
            MessageBox.Show("Please enter release date no!");
    ...
    protected void Load_LoanRelease(int LoanRelease_Id)
    {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            txtReleaseAmount.Text = dt.Rows[0]["ReleaseAmount"].ToString();
            txtInstNo.Text = dt.Rows[0]["InstNo"].ToString();

           // DateTime releaseDate = Convert.ToDateTime(dt.Rows[0]["ReleaseDate"]);
            txtReleaseDate.Text = dt.Rows[0]["ReleaseDate"].ToString().ToDate().ToString("dd/MMM/yyyy");
