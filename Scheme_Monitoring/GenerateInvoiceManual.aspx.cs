...
public partial class GenerateInvoiceManual : System.Web.UI.Page
{
    ...
    private void reset_DE()
    {
        ...
        txtRABillNo.Text = "";
        txtTotalAmount.Text = "";
        ChkSkipDeduction.Checked = false;
        ChkSkipDeduction_CheckedChanged(ChkSkipDeduction, new EventArgs());
        get_tbl_Deduction1();
    }
    ...
    protected void btnViewDeduction_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (Invoice_Id > 0)
        {
            ...
            else
            {
                grdDeductionHistory.DataSource = null;
                grdDeductionHistory.DataBind();
                MessageBox.Show("Deduction Details Not Found");
            }
        }
    }
    ...
    {
        DataSet ds = new DataSet();
        ds = (new DataLayer()).get_tbl_TradeDocumentManualUpload();
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
    ...
    protected void ChkSkipDeduction_CheckedChanged(object sender, EventArgs e)
    {
        DivDeductionDetails.Visible = !ChkSkipDeduction.Checked;
    }
