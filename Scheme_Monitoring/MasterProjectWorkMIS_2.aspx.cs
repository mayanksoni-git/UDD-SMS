...
public partial class MasterProjectWorkMIS_2 : System.Web.UI.Page
{
    ...
    private void get_tbl_ProjectWorkGO(int ProjectWork_Id)
    {
        ...
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            ...
            grdCallProductDtls.FooterRow.Cells[4].Text = ds.Tables[0].Compute("sum(ProjectWorkGO_CentralShare)", "").ToString();
            grdCallProductDtls.FooterRow.Cells[5].Text = ds.Tables[0].Compute("sum(ProjectWorkGO_StateShare)", "").ToString();
            grdCallProductDtls.FooterRow.Cells[6].Text = ds.Tables[0].Compute("sum(ProjectWorkGO_Centage)", "").ToString();
            grdCallProductDtls.FooterRow.Cells[7].Text = ds.Tables[0].Compute("sum(GO_Total)", "").ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < grdCallProductDtls.Rows.Count; i++)
        {
            ...
            Allocated_Budget = CentralShare_Budget + StateShare_Budget + Centage_Budget;
            ...
            {
                MessageBox.Show("Please Fill GO Amount");
                return;
            }
        }
    ...
    protected void grdCallProductDtls_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                LinkButton lnkBtn = (LinkButton)e.Row.FindControl("lnkFRID");
                lnkBtn.Visible = false;
            }
        }
    }
