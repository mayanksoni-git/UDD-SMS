var unitList, checkAll = document.getElementById("checkAll"),
    ...
    editlist = !1,
    options = {
        valueNames: ["id", "vidha_name", "unit_name", "unit_shortname", "created_date", "status"],
        page: perPage,
        pagination: !0,
...
document.getElementById("unitList") && (unitList = new List("unitList", options).on("updated", function (e) {
...
//            order: "desc"
//        }), refreshCallbacks()
//    }), unitList.remove("id", '<a href="javascript:void(0);" class="fw-medium link-primary">#VZ2101</a>')
//},
//    xhttp.open("GET", "../Content/assets/json/table-customer-list.json"), xhttp.send();


var isValue = (isCount = (new DOMParser).parseFromString(unitList.items.slice(-1)[0]._values.id, "text/html")).body.firstElementChild.innerHTML,
...
refreshCallbacks(), document.getElementById("showModal") && (document.getElementById("showModal").addEventListener("show.bs.modal", function (e) {
    e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("labelModalPopUp").innerHTML = "Edit Customer", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("labelModalPopUp").innerHTML = "Add Role", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Role") : (document.getElementById("labelModalPopUp").innerHTML = "List Customer", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
}), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function () {
    clearFields()
})), document.querySelector("#unitList").addEventListener("click", function () {
    ischeckboxcheck()
});
...
var table = document.getElementById("unitTable"),
    ...
    statusVal = (Array.prototype.slice.call(forms).forEach(function (a) {
        a.addEventListener("submit", function (e) {
            var t;
            a.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === emailField.value || "" === dateField.value || "" === phoneField.value || editlist ? "" !== customerNameField.value && "" !== emailField.value && "" !== dateField.value && "" !== phoneField.value && editlist && (t = unitList.get({
                id: idField.value
                ...
                position: "center",
                icon: "success",
                title: "Role inserted successfully!",
                showConfirmButton: !1,
                timer: 2e3,
                showCloseButton: !0
        ...
        e.addEventListener("click", function (e) {
            ...
            Array.from(e).forEach(function (e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function () {
                    unitList.remove("id", t.outerHTML), document.getElementById("deleteRecordModal").click()
                })
            })
        })
...
function deleteMultiple() {
    ...
    if (Array.from(e).forEach(function (e) {
        ...
        Array.from(ids_array).forEach(function (e) {
            unitList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${e}</a>`)
